...
::: 
:::caution
The creation of a ReadCursor will error if the size of the buffer exceeds 1 000 000 (e.g. when exploiters send large buffers), handle this accordingly if the following code should still run.
:::
...
|u40|5|0|1 099 511 627 775|
|u48|6|0|281 474 976 710 655|
...
|i40|5|-549 755 813 888|549 755 813 887|
|i48|6|-140 737 488 355 328|140 737 488 355 327|
...
:::note
Booleans in structs will be automatically bitpacked (up to 32), meaning 8 booleans together take up 1 byte.
:::

:::caution
If you want to use bools in structs, use bool_simple instead of bool, as bool returns a tuple which results in only the first bool being written/read in practice. Otherwise the other 7 bits are still calculated, wasting performance.
Additionally, only bool_simple will be considered for bitpacking, so if you want this optimization to kick in, use bool_simple.
:::
...
:::note
Optionals will also be bitpacked like booleans (again, up to 32), meaning 8 optional fields only take up 1 extra byte.
:::
